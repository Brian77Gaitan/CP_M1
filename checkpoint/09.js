...
BinarySearchTree.prototype.consultarTragos = function (efectivo, contenido = []) {
  
  if (this.value.precio <= efectivo) {
    contenido.push(this.value.nombre);
  }
  if (this.left) {
    this.left.consultarTragos(efectivo, contenido);
  }
  if (this.right) {
    this.right.consultarTragos(efectivo, contenido);
  }
  return contenido;
}
